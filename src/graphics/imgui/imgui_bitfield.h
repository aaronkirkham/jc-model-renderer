...
// credits to https://github.com/JSandusky
namespace ImGuiCustom
{
template <typename T> bool BitField(const char* label, T* bits, unsigned* hoverIndex)
{
    unsigned val = *bits;
    ...
    for (unsigned i = 0; i < (sizeof(T) * 8); ++i) {
        ...
        const ImRect check_bb(currentPos, {currentPos.x + smallLabelSize.y + style.FramePadding.y * 2,
                                           currentPos.y + smallLabelSize.y + style.FramePadding.y * 2});
        ...
        if (*bits & (1 << i)) {
            ...
            window->DrawList->AddRectFilled({check_bb.Min.x + pad, check_bb.Min.y + pad},
    ...
    const ImRect matrix_bb(
        window->DC.CursorPos,
        {window->DC.CursorPos.x + (smallLabelSize.y + style.FramePadding.y * 2) * 16 /*# of checks in a row*/
             + smallLabelSize.y /*space between sets of 8*/ + 15 * spacingUnit /*spacing between each check*/,
         window->DC.CursorPos.y
             + ((smallLabelSize.y + style.FramePadding.y * 2) * 2 /*# of rows*/
                + spacingUnit /*spacing between rows*/)});
...
bool BitFieldTooltip(const char* label, T* bits, const std::array<const char*, N>& hover_labels)
{
    uint32_t hover_index = -1;
    auto     result      = BitField(label, bits, &hover_index);

    if (hover_index != -1 && hover_index < N) {
        const auto& label = hover_labels[hover_index];
        if (strlen(label) > 0) {
